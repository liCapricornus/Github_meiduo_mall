...
class UserHistoryView(LoginRequiredJSONMixin,View):
    ...
    def post(self,request):
        ...
        # get key: (error) WRONGTYPE Operation against a key holding the wrong kind of value
        # 大哥 get key 是redis_string 获取value
        # redis_list 为 lrange key start stop 如：lrange history_1 0 -1 为显示全部value
        # 只保存5条数据
        redis_cli.ltrim('history_%s'%user.id,0,4)
        # 5.返回响应
        return JsonResponse({'code':0,'errmsg':'add user history OK!'})
    ...
    def get(self,request):
        ...
        user = request.user
        redis_cli = get_redis_connection('user_history')
        # key = redis_cli.keys *  获取 key
        key = "history_%s"%user.id
        sku_ids = redis_cli.lrange(key,0,4)
        # 2.验证参数
        if not all([key,sku_ids]):
            return JsonResponse({'code':400,'errmsg':'参数不全！'})
        ...
        # 4.返回响应
        return JsonResponse({'code':0,'errmsg':'OK!','skus':sku_history_list})
